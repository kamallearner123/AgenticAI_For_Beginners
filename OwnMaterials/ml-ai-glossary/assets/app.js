...
function render(){
  ...
  for (const t of filtered){
    ...
    card.className='card';
    const icon = '📘';
    const slug = t.term.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,"")||"term";
const href = `pages/term-${t.id.toString().padStart(3,"0")}-${slug}.html`;
card.setAttribute('data-href', href);
card.innerHTML = `
      <a class="invisible-link" href="${href}" aria-label="Open ${t.term}"></a>
      <div class="top">
        <span class="badge">${t.category}</span>
        <button class="bookmark" data-id="${t.id}" title="Bookmark">${state.bookmarks.has(t.id) ? '★' : '☆'}</button>
      </div>
      <h4><a href="${href}">${t.term}</a></h4>
...
document.addEventListener('click', (e)=>{
  ...
  if(card && card.dataset.href && !e.target.closest('button') && !e.target.closest('a')){
    window.location.href = card.dataset.href;
  }
});


// ----- Navigation Enhancer (robust) -----
(function(){
  if (!window._renderWrapped){
    window._renderWrapped = true;
  ...
  function postRender(){
    // attach data-href from title or by lookup in TERMS
    ...
    for (const card of cards){
      ...
      if (!href){
        ...
        if (title){
          const t = TERMS.find(x=>x.term===title);
          if (t){
            const slug = t.term.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,'' ) || 'term';
            href = `pages/term-${String(t.id).padStart(3,'0')}-${slug}.html`;
            // also insert title link if missing
            const h4 = card.querySelector('h4');
            if (h4) h4.innerHTML = `<a href="${href}">${title}</a>`;
          }
        }
      }
      if (href){
        card.dataset.href = href;
        // Make entire card clickable (but ignore buttons/links)
  ...
  // run once in case render already happened
